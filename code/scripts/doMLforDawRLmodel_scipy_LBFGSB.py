...
def main(argv):
    ...
    parser.add_argument("--optim_maxiter", 
    ...
    params = [alpha_0, beta_stage2_0, beta_MB_0, beta_MF0_0, beta_MF1_0,
              beta_stick_0]
    bounds = [alpha_bounds, beta_stage2_bounds, beta_MB_bounds, beta_MF0_bounds,
              beta_MF1_bounds, beta_stick_bounds]
    x0 = np.array(params)

    dawRLmodel = models.DawRLmodel(subject_data=subject_data,
    ...
    def nLogLikelihood(x):
        dawRLmodel.params = x
        nLL = -dawRLmodel.logLikelihood()
        return nLL
    eval_func_wrapped = utils.wrap_torch_to_numpy_func(torch_func=
                                                       nLogLikelihood)
    optim_params = {'maxiter': maxiter}

    min_res = optim.minimize_scipy_LBFGSB(func=eval_func_wrapped, x0=x0,
                                          bounds=bounds,
