...
sys.path.append("../src")
import models_autogradNumpy as models
import optim
import utils
...
def main(argv):
    ...
    params = [alpha_0, beta_stage2_0, beta_MB_0, beta_MF0_0, beta_MF1_0,
              beta_stick_0]
    bounds = [(0, 1), (-anp.inf, anp.inf), (-anp.inf, anp.inf),
              (-anp.inf, anp.inf), (-anp.inf, anp.inf), (-anp.inf, anp.inf)]
    x = [anp.array(params, dtype=anp.double)]
    ...
    # max_res = optim.maximize_LBFGSB(func=eval_func, x=x[0], bounds=bounds, optim_params=optim_params)
    def eval_func(x):
        # import pdb; pdb.set_trace()
        dawRLmodel.params = x
        nELL = -dawRLmodel.complete_ll_expectation()
        return nELL
    max_res = optim.maximize_LBFGSB(func=eval_func, x0=x[0], bounds=bounds, optim_params=optim_params)
    pdb.set_trace()
